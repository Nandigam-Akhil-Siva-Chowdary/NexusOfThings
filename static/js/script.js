...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initEventModals() {
    const eventButtons = document.querySelectorAll('.event-detail-btn, .btn-neon[data-event], .event-card-footer .btn');
    
    eventButtons.forEach(button => {
      button.addEventListener('click', function() {
        const eventName = this.getAttribute('data-event') || this.closest('.event-card')?.querySelector('h3')?.textContent;
        if (eventName) {
  ...
  function renderEventDetails(data) {
    const studentCards = (data.student_coordinators || [])
      .map(
        ...
        <div class="coordinator-card glass" style="margin-bottom: 15px;">
          <h4 style="color: var(--accent-color); font-family: 'Orbitron', sans-serif; margin-bottom: 10px;">Student Coordinator ${idx + 1}</h4>
          <p><strong style="color: var(--text-primary);">Name:</strong> <span style="color: var(--text-secondary);">${c.name || 'TBD'}</span></p>
          <p><strong style="color: var(--text-primary);">Roll No:</strong> <span style="color: var(--text-secondary);">${c.roll_number || 'N/A'}</span></p>
          <p><strong style="color: var(--text-primary);">Phone:</strong> <span style="color: var(--text-secondary);">${c.phone || 'N/A'}</span></p>
        </div>
      `
      )
      .join('');
    ...
    const facultyCard = `
      <div class="coordinator-card glass" style="margin-bottom: 15px;">
        ...
        <p><strong style="color: var(--text-primary);">Name:</strong> <span style="color: var(--text-secondary);">${data.faculty_coordinator_name || 'Faculty Coordinator'}</span></p>
        <p><strong style="color: var(--text-primary);">Designation:</strong> <span style="color: var(--text-secondary);">${data.faculty_coordinator_designation || 'TBD'}</span></p>
        <p><strong style="color: var(--text-primary);">Phone:</strong> <span style="color: var(--text-secondary);">${data.faculty_coordinator_phone || 'N/A'}</span></p>
      </div>
    `;

    modalBody.innerHTML = `
      <div class="event-details">
        <h2 style="color: var(--accent-color); font-family: 'Orbitron', sans-serif; margin-bottom: 20px; text-align: center;">${data.title}</h2>
        ...
        <div class="event-info-section glass" style="padding: 20px; border-radius: 15px; margin-bottom: 20px;">
          ...
          <p style="color: var(--text-secondary); line-height: 1.8;">${data.description || 'Details will be updated soon.'}</p>
        </div>
        
        <div class="event-info-section glass" style="padding: 20px; border-radius: 15px; margin-bottom: 20px;">
          <h3 style="color: var(--accent-color); margin-bottom: 15px; font-family: 'Orbitron', sans-serif;">Event Structure</h3>
          <p style="color: var(--text-secondary); line-height: 1.8;">${data.rounds_info || 'Structure will be shared soon.'}</p>
        </div>
        
        <div class="event-info-section glass" style="padding: 20px; border-radius: 15px; margin-bottom: 20px;">
          <h3 style="color: var(--accent-color); margin-bottom: 15px; font-family: 'Orbitron', sans-serif;">Rules & Guidelines</h3>
          <p style="color: var(--text-secondary); line-height: 1.8;">${data.rules || 'Rules will be announced shortly.'}</p>
        </div>
        
        <div class="event-info-section glass" style="padding: 20px; border-radius: 15px; margin-bottom: 20px;">
          <h3 style="color: var(--accent-color); margin-bottom: 15px; font-family: 'Orbitron', sans-serif;">Team Requirements</h3>
          <p style="color: var(--text-secondary); line-height: 1.8;">${data.team_requirements || 'Refer to rules for team size.'}</p>
        </div>
        
        <div class="event-info-section glass" style="padding: 20px; border-radius: 15px; margin-bottom: 20px;">
          <h3 style="color: var(--accent-color); margin-bottom: 15px; font-family: 'Orbitron', sans-serif;">Prizes</h3>
          <p style="color: var(--text-secondary); line-height: 1.8;">${data.prizes || 'Prize details will be announced during the event.'}</p>
        </div>
        ...
        <div class="event-info-section glass" style="padding: 20px; border-radius: 15px; margin-bottom: 20px;">
          ...
          <div class="coordinator-details" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            ${facultyCard}
            ${studentCards || '<div class="coordinator-card glass"><p style="color: var(--text-secondary); text-align: center;">Student coordinators will be updated shortly.</p></div>'}
          </div>
        </div>
      ...
      ">
        <div style="display: flex; align-items: center; gap: 15px;">
          <i class="fas ${type === 'success' ? 'fa-check-circle' : type === 'error' ? 'fa-exclamation-circle' : 'fa-info-circle'}" 
             style="color: ${type === 'success' ? 'var(--success-color)' : type === 'error' ? 'var(--danger-color)' : 'var(--accent-color)'}; font-size: 1.5rem;"></i>
